...
from correlation import (
    compute_eta_trip, compute_eta_pt, compute_eta_proximity,
    _build_station_pt_proximity, _build_station_pt_min_dist,
)
...
def run_sensitivity_analysis(
    ...
    print(f"\n  Policy parameters held fixed: w1={w1}, w2={w2}")
    print(f"  Deltas: {config.SENSITIVITY_DELTAS}")
    print(f"  Realisations per delta: {config.SENSITIVITY_N_REALISATIONS}\n")

    print("  Precomputing station-PT proximity for sensitivity metrics ...")
    ...
    for delta in config.SENSITIVITY_DELTAS:
        eta_trip_vals, eta_pt_vals, eta_prox_vals, solve_time_vals = [], [], [], []
        ...
        all_solve_times.extend(solve_time_vals)

        print(
            f"delta={delta:.1f}  (n={n_solved:2d})  "
            f"η_trip={record['eta_trip_mean']:.3f} +- {record['eta_trip_std']:.3f}  "
            f"η_pt={record['eta_pt_mean']:.3f} +- {record['eta_pt_std']:.3f}  "
            f"η_proximity={record['eta_proximity_mean']:.1f} +- {record['eta_proximity_std']:.1f} m"
        )

    return pd.DataFrame(records), all_solve_times
